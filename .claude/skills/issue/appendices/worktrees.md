...
~/Development/
├── amara/    # Main repo
├── amara-294/                  # Worktree for issue #294
├── amara-295/                  # Worktree for issue #295
└── amara-301/                  # Worktree for issue #301
```
...
```bash
export COMPOSE_PROJECT_NAME=amara-294
...
├── Tracks state in ~/.amara/portfolio/<num>/
...
└── No-op when AMARA_ISSUE_NUM not set (safe for non-portfolio sessions)
...
This is by design. `portfolio-notify.sh` is a no-op when `AMARA_ISSUE_NUM` is not set, so it silently exits in non-portfolio sessions.
