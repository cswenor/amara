...
- [ ] `/issue <num>` in START mode from main repo creates worktree at `../amara-<num>/`
...
- [ ] `tmux-session.sh init` creates session `amara` with `main` window
...
- [ ] Hooks fire and update `~/.amara/portfolio/<num>/status`
- [ ] `portfolio-notify.sh` is a no-op when `AMARA_ISSUE_NUM` not set
