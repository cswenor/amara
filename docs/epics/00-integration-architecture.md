...
2. [Queue]     Event written to Amara Event Queue with mode: "monitored"
                ...
                → If orchestrator needs to send a response in the monitored channel,
                  it checks D14 write permission (per-instruction or standing rule)
...
| Channel Mode | Read | Silent Actions (archive, label, mark read, draft) | Send Messages |
|---|---|---|---|
| **Direct** | always | always | always — this is Amara's conversation channel |
| **Monitored** (default) | always | always (triage layer) — includes draft creation | **never** — unless explicitly authorized |
| **Monitored + write grant** | always | always (triage layer) — includes draft creation | only via orchestrator, only when user has granted permission |

**How write permission is granted for monitored channels:**

1. **Per-instruction** — user tells Amara via direct channel: "Reply to John in our group chat saying I'll be late." The orchestrator executes this as a task, sends the message in that specific conversation, and the grant expires after the task completes. Grant is scoped to the target channel/thread.
...
**Silent actions vs outbound messaging:** Drafting (Gmail drafts, unsent message composition) is classified as a silent action — it does not deliver content to external recipients and does not require a write grant. Only actions that deliver content externally (send message, post reply) require authorization. Archive, label, mark read, and draft are all silent actions.
